<?php
ob_start();
$cookie_ref = "referred_by";

if(!isset($_COOKIE[$cookie_ref])) {
    if(!isset($_GET['ref'])) { $referby='None'; } else { $referby=filterData($_GET['ref']); }
setcookie($cookie_ref, $referby);
    $ref=$referby;
} else {
    $ref=$_COOKIE[$cookie_ref];
}
ob_end_flush();
?><?php
...
$refern = $_POST['refname'];
$withdrawn = $_POST['bitaddress'];
$password = filterData($_POST['password']);
...
{
    $core_system_messages[] = "Bitcoin Address is empty";
} else {
...
mysqli_query($con,"UPDATE statistics SET value = value+1 WHERE name = 'Members'");
$stmt = mysqli_stmt_init($con);
$stmt = mysqli_prepare($con,"INSERT INTO users (user_name,user_email,password_hash,user_verified,activation_hash,registration_datetime,registration_ip,withdrawaddress,referby) VALUES (?,?,?,?,?,?,?,?,?)");
mysqli_stmt_bind_param($stmt,"sssisssss",$username,$email,$password_hash,$user_verified,$activation_hash,$registration_datetime,$registration_ip,$withdrawn,$refern);
mysqli_stmt_execute($stmt);
mysqli_stmt_close($stmt);
...
$username = filterData($_GET['username']);
   ...
   $u_data = mysqli_fetch_array($result,MYSQLI_ASSOC);
